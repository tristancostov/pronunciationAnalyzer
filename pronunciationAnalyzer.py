...
# Игнорируем предупреждение librosa о пустых фильтрах для коротких фрагментов
...
def computeContours(wordSegment: np.ndarray, sr: int):
    ...
    Возвращает по-кадрово (один и тот же hop для всех кривых):
      ...
      mfcc        — матрица кепстральных коэффициентов (добавлено для поиска границ)
    """
    ...
    return energyDb, voiced, vowelScore, hopLen, mfcc
...
def analyzeSyllablesInWord(wordSegment: np.ndarray, sr: int,
    ...
    # ── ИЗМЕНЕНО: распаковываем mfcc ──
    energyDb, voiced, vowelScore, hopLen, mfcc = computeContours(core, sr)
