...
DIP_DB              = 3.0      
PEAK_FLOOR_DB       = 30.0     
MIN_NUCLEUS_DIST_S  = 0.080    

VOICE_LOWBAND_RATIO = 0.45     
VOICE_ZCR_MAX       = 0.18     

REDUCIBLE_VOWELS    = set("аоеяи")
DUR_RATIO_NO_RED    = 0.85     
ENER_RATIO_NO_RED   = 0.80     
SCHWA_F1            = 500.0   
SCHWA_F2            = 1500.0
...
def computeContours(wordSegment: np.ndarray, sr: int):
    ...
    n = min(len(rmsLow), len(rmsFull), len(zcr))
...
def selectNuclei(energyDb: np.ndarray, voiced: np.ndarray, expectedCount: int,
    ...
    score = _norm(energyDb[cand]) + _norm(props["prominences"])
    if len(cand) >= expectedCount:
        keep = cand[np.argsort(score)[-expectedCount:]]   
        return np.sort(keep), rawCount
    return np.sort(cand), rawCount                         


def nucleiToBoundaries(peaks: np.ndarray, energyDb: np.ndarray, hopLen: int,
                       totalSamples: int, sr: int, expectedCount: int) -> list[tuple[int, int]]:
...
def analyzeSyllablesInWord(wordSegment: np.ndarray, sr: int,
    ...
    energyDb, voiced, hopLen = computeContours(wordSegment, sr)
    peaks, rawDetected = selectNuclei(energyDb, voiced, expected, hopLen, sr)
    bounds = nucleiToBoundaries(peaks, energyDb, hopLen, len(wordSegment), sr, expected)
...
def analyzeVowelReduction(syllableResults: list[dict], syllables: list[str],
    ...
    for i, (sd, sylText) in enumerate(zip(syllableResults, syllables)):
        ...
        # нет редукции: безударный почти такой же длинный И громкий
        if durRatio >= DUR_RATIO_NO_RED and enerRatio >= ENER_RATIO_NO_RED:
            errors.append({
                "syllable": sylText, "position": i + 1,
                "durRatio": round(durRatio, 2), "enerRatio": round(enerRatio, 2),
                "hint": (f"Безударный слог «{sylText}» по длине {durRatio*100:.0f}% и "
                         f"по энергии {enerRatio*100:.0f}% от ударного «{syllables[stressedIdx]}». "
                         f"Похоже, гласный не редуцирован."),
            })
        else:
            correct += 1
...
def main():
    ...
    for wd in wordDetails:
        ...
        if accentizer is not None and surface:
            stressedIdx = findStressedSyllableByDict(surface, syllables, accentizer)
        if not (0 <= stressedIdx < len(syllableResults)):  # резерв: по энергии
            ens = [r["acoustics"].get("energyMean", 0) for r in syllableResults]
            stressedIdx = int(np.argmax(ens)) if ens else 0
